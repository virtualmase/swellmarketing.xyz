...
function sourceDetail(event) {
  ...
  return [
    attribution.utmCampaign && `first_campaign=${attribution.utmCampaign}`,
    attribution.utmContent && `first_content=${attribution.utmContent}`,
    attribution.referrer && `first_referrer=${attribution.referrer}`,
    latest.utmCampaign && `latest_campaign=${latest.utmCampaign}`,
    latest.utmContent && `latest_content=${latest.utmContent}`,
    latest.sourcePage && `latest_page=${latest.sourcePage}`
  ].filter(Boolean).join(" | ").slice(0, 1000);
}

function funnelMilestoneProperties(event) {
  const propertyByEvent = {
    "meeting.booked": "swell_meeting_booked_at",
    "meeting.held": "swell_meeting_held_at",
    "meeting.no_show": "swell_meeting_no_show_at",
    "proposal.sent": "swell_proposal_sent_at",
    "proposal.accepted": "swell_proposal_accepted_at"
  };
  const property = propertyByEvent[event.eventType];
  return property ? { [property]: event.occurredAt } : {};
}

const FUNNEL_MILESTONE_EVENTS = new Set([
  "meeting.booked",
  "meeting.held",
  "meeting.no_show",
  "proposal.sent",
...
export function contactProperties(event, existing, ownerId) {
  ...
  const hasResponseConsent = consent.response !== undefined || contact.followUpConsent !== undefined;
  const hasMarketingConsent = consent.marketing !== undefined;
  return compact({
    ...
    swell_source: opportunity.source || (existing ? undefined : "unknown"),
    swell_source_detail: sourceDetail(event) || undefined,
    swell_inquiry_origin: opportunity.inquiryOrigin || (existing ? undefined : "general"),
    swell_inquiry_origin_detail: opportunity.inquiryOriginDetail || undefined,
    swell_first_constraint: opportunity.firstConstraint || (existing ? undefined : "unclear"),
    swell_trigger: opportunity.trigger,
    swell_commercial_consequence: opportunity.commercialConsequence,
    swell_timeline: opportunity.timeline || (existing ? undefined : "unknown"),
    swell_fit: opportunity.fit || (existing ? undefined : "unknown"),
    swell_recommended_offer: opportunity.recommendedOffer || (existing ? undefined : "unknown"),
    swell_response_consent: hasResponseConsent ? bool(consent.response === true || contact.followUpConsent === true) : undefined,
    swell_marketing_consent: hasMarketingConsent ? bool(consent.marketing === true) : undefined,
    ...
    ...funnelMilestoneProperties(event)
  });
}
...
function noteBody(event) {
  ...
  const rows = [
    ...
    ["Inquiry origin", opportunity.inquiryOrigin],
    ["Inquiry origin detail", opportunity.inquiryOriginDetail],
...
function dealProperties(event, pipeline, stage, ownerId) {
  ...
  return compact({
    ...
    swell_inquiry_origin: opportunity.inquiryOrigin || "general",
    swell_inquiry_origin_detail: opportunity.inquiryOriginDetail,
...
export async function processHubSpotEvent(event, {
  ...
  let deal = null;
  const shouldCreateDeal = event.opportunity?.fit === "qualified" && !FUNNEL_MILESTONE_EVENTS.has(event.eventType);
  const warnings = [];
  if (shouldCreateDeal) {
