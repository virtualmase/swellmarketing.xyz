  ...
  return (actions[constraint.id] || []).slice(0, 3).map(([title, outcome, metric, dueInDays]) => {
    ...
    return {
      ...
      blocker: baselineBlocked
        ? snapshot.evidenceCoverage?.answerSurfaceAdaptersConfigured > 0
          ? "The controlled Perplexity Sonar API adapter is verified, but its credential is not available to this runtime yet. A Sonar API run will be labeled separately from the consumer Perplexity Pro application."
...
export function buildOfflineBrief({ agent, snapshot, now = new Date() }) {
  ...
  const answerAdapterConfigured = baselinePrepared && snapshot.evidenceCoverage?.answerSurfaceAdaptersConfigured > 0;
  return {
    ...
    draft: {
      ...
      body: baselinePrepared
        ? `${snapshot.evidenceCoverage?.campaignPublications > 0 ? "The first campaign article is live in production." : "The first campaign asset is prepared."} The ten-account research set and 30-query observation manifest are prepared.${answerControlCompleted ? " The public-search control is complete and narrowed the premise: current first-party evidence surfaced for 29 questions, while five descriptions were mixed or incomplete; this is not AI-answer evidence." : ""} No researched account has a documented permitted-contact basis. ${answerAdapterConfigured ? "The controlled Perplexity Sonar API adapter is verified, but its credential is not available to this runtime; the Google adapter is absent and OpenAI credits remain unavailable." : "The OpenAI API has no remaining credits, and no reproducible Google or Perplexity observation adapter is configured."} No organic social publishing adapter is connected. Do not convert relevance into implied permission or convert an unobserved query into a representation claim.`
